...
internal abstract class Repository<T> : QueryHooks<T>, IRepository<T> where T : BaseEntity
{
    ...
    public virtual async Task<T?> GetByIdAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = BuildQuery(_context, new QueryObject());
    ...
    public virtual async Task<IEnumerable<T>> SearchAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = BuildQuery(_context, searchQuery);
        query = ApplyFilters(query, searchQuery, userId);
        query = ApplySorting(query, searchQuery);

        int skip = (searchQuery.Page - 1) * searchQuery.PageSize;
